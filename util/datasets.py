...
import os, io, argparse, pickle, math, lmdb, multiprocessing as mp
...
def build_dataset(is_train, args):
    ...
    elif args.data_set == 'INREC':
        # print('build REC_IMAGENET_1K')
        # nb_classes = 1000
        # rec_data_path = os.path.join(args.data_path, 'ImageNet_rec')
        # data_path = os.path.join(rec_data_path, 'train') if is_train else os.path.join(rec_data_path, 'val')
        # dataset = make_dataset(data_path=data_path,
    ...
    elif args.data_set == 'INLMDB':
        if is_train:
            data_path = os.path.join(args.data_path, 'imagenet_train.lmdb')
        else:
            data_path = os.path.join(args.data_path, 'imagenet_val.lmdb')

        dataset = ImageNetLMDB(lmdb_path = data_path, transform=transform)
        nb_classes = 1000
