...
def _load_dinov2_local(model_config: str, use_reg: bool, device, resolution: int):
    """
    从本地 dinov2 仓库加载权重，不依赖 torch hub 缓存/网络。
    目录优先级：$DINOV2_PATH > ./dinov2
    """
    repo_dir = os.environ.get("DINOV2_PATH", os.path.join(os.path.dirname(__file__), "dinov2"))
    if not os.path.isdir(repo_dir):
        raise RuntimeError(f"dinov2 repo not found at: {repo_dir}. Please `git clone` it.")

    # 让 python 能 import 到 dinov2 包
    if repo_dir not in sys.path:
        sys.path.insert(0, repo_dir)

    # 用 torch.hub 从本地 hubconf 加载（不会访问网络/缓存）
    entry = f"dinov2_vit{model_config}14_reg" if use_reg else f"dinov2_vit{model_config}14"
    encoder = torch.hub.load(repo_dir, entry, source="local", pretrained=True)

    # 去掉 head，并按当前分辨率重采样绝对位置编码
    import timm
    if hasattr(encoder, "head"):
        encoder.head = torch.nn.Identity()
    patch_resolution = 16 * (resolution // 256)
    if hasattr(encoder, "pos_embed") and hasattr(encoder.pos_embed, "data"):
...
def load_encoders(enc_type, device, resolution=256):
    ...
    for enc_name in enc_names:
        ...
        if encoder_type == 'mocov3':
            ...
            # 本地优先加载（不使用 cache，不访问网络）
            use_reg = ('reg' in encoder_type)
            encoder = _load_dinov2_local(model_config, use_reg, device, resolution)
