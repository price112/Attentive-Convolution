...
class ATConv(nn.Module):
    ...
    def __init__(self,
                 dim,
                 act_layer = nn.SiLU, # consistent with SiT
                 kernel_size = 3,
                 bias = True,):
        super().__init__()
    ...
    def _generate_kernels(self, x: torch.Tensor):
...
class SiTBlock(nn.Module):
    ...
    def __init__(self, hidden_size, num_heads, mlp_ratio=4.0, use_atconv = False, **block_kwargs):
        super().__init__()
        self.norm1 = nn.LayerNorm(hidden_size, elementwise_affine=False, eps=1e-6)

        if use_atconv:
            self.attn = ATConv(dim=hidden_size)
        else:
            self.attn = Attention(
...
class SiT(nn.Module):
    ...
    def __init__(
        ...
        self.blocks = nn.ModuleList([
            SiTBlock(hidden_size, num_heads, mlp_ratio=mlp_ratio, use_atconv = use_atconv if ((_+1)%2 == 0 and hybrid) else False, **block_kwargs) for _ in range(depth)
        ])
...
def SiT_XL_2_ATConv(**kwargs):
    return SiT(depth=28, hidden_size=1152, decoder_hidden_size=1152, patch_size=2, num_heads=16, use_atconv = True, hybrid = False, **kwargs)

def SiT_XL_2_ATConv_Hybrid(**kwargs):
    return SiT(depth=28, hidden_size=1152, decoder_hidden_size=1152, patch_size=2, num_heads=16, use_atconv = True, hybrid = True, **kwargs)


def SiT_L_2(**kwargs):
    return SiT(depth=24, hidden_size=1024, decoder_hidden_size=1024, patch_size=2, num_heads=16, **kwargs)

def SiT_L_2_ATConv(**kwargs):
    return SiT(depth=24, hidden_size=1024, decoder_hidden_size=1024, patch_size=2, num_heads=16, use_atconv = True, hybrid = False, **kwargs)

def SiT_L_2_ATConv_Hybrid(**kwargs):
    return SiT(depth=24, hidden_size=1024, decoder_hidden_size=1024, patch_size=2, num_heads=16, use_atconv = True, hybrid = True, **kwargs)


def SiT_B_2(**kwargs):
    return SiT(depth=12, hidden_size=768, decoder_hidden_size=768, patch_size=2, num_heads=12, **kwargs)

def SiT_B_2_ATConv(**kwargs):
    return SiT(depth=12, hidden_size=768, decoder_hidden_size=768, patch_size=2, num_heads=12, use_atconv = True, hybrid = False, **kwargs)

def SiT_B_2_ATConv_Hybrid(**kwargs):
    return SiT(depth=12, hidden_size=768, decoder_hidden_size=768, patch_size=2, num_heads=12, use_atconv = True, hybrid = True, **kwargs)

SiT_models = {
    'SiT-XL/2': SiT_XL_2,
    'SiT-L/2': SiT_L_2,
    'SiT-B/2': SiT_B_2,
    'SiT-B/2-ATConv': SiT_B_2_ATConv,
    'SiT-L/2-ATConv': SiT_L_2_ATConv,
    'SiT-XL/2-ATConv': SiT_XL_2_ATConv,
    'SiT-B/2-ATConv-Hybrid': SiT_B_2_ATConv_Hybrid,
    'SiT-L/2-ATConv-Hybrid': SiT_L_2_ATConv_Hybrid,
    'SiT-XL/2-ATConv-Hybrid': SiT_XL_2_ATConv_Hybrid,
}