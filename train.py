...
def get_args_parser():
    parser = argparse.ArgumentParser(
        'AttNet training and evaluation script', add_help=False)
    parser.add_argument('--batch_size', default=1024, type=int)
    parser.add_argument('--epochs', default=100, type=int)

    # Model parameters
    parser.add_argument('--model', default='FreeNets', type=str, metavar='MODEL',
    ...
    parser.add_argument('--weight_decay', type=float, default=0.01,
                        help='weight decay (default: 0.025 for adamw)')
    ...
    parser.add_argument('--distillation-type', default='none',
                        choices=['none', 'soft', 'hard','dkd', 'none'], type=str, help="default :none")
    ...
    parser.add_argument('--data_path', default='data/', type=str,
                        help='dataset path')
    parser.add_argument('--data-set', default='INLMDB', choices=['CIFAR', 'IMNET', 'INAT', 'INAT19', 'INREC', 'INLMDB'],
    ...
    parser.add_argument('--log_freq', default=[250, 280], type=list,
                        help='log frequency for every 10/5/1 epoches')
...
def main(args):
    ...
    utils.setup_print_for_distributed()
    ...
    # save args settings to the log file
    if args.output_dir and utils.is_main_process():
        with (output_dir / "log.txt").open("a") as f:
            for kk, vv in sorted(vars(args).items()):
                str_args = str(kk) + '=' + str(vv) + "\n"
                f.write(str_args)
    ...
    # os.environ["MIOPEN_CACHE_DIR"] = "/dev/null"
    ...
    # Will degenerate into general cross entropy loss
    ...
    if args.eval:
        test_stats = evaluate(data_loader_val, model, local_rank)
        print(
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(
            ...
            optimizer, local_rank, epoch, loss_scaler,
        ...
        if (epoch <= args.log_freq[0] and epoch % 10 == 0) or (
                args.log_freq[0] < epoch <= args.log_freq[1] and epoch % 5 == 0) or (epoch > args.log_freq[1]):
            test_stats = evaluate(data_loader_val, model, local_rank)
            print(
                f"Accuracy of the network on the {len(dataset_val)} test images: {test_stats['acc1']:.1f}%")
            max_accuracy = max(max_accuracy, test_stats["acc1"])

            checkpoint_paths = [output_dir / (args.model + '_' + str(epoch) + '_' + str(test_stats["acc1"]) + '.pth')]
            for checkpoint_path in checkpoint_paths:
                utils.save_on_master({
                    'model': model_without_ddp.state_dict(),
                    'optimizer': optimizer.state_dict(),
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        'SCFormer training and evaluation script', parents=[get_args_parser()])
